...
/**
 * 检查指定 Python 环境是否安装了 cutile-lsp
 * @param pythonExecutable Python 解释器路径
 * @returns 是否安装
 */
function isCutileLspInstalled(pythonExecutable: string): boolean {
    try {
        execSync(`${pythonExecutable} -c "import cutile_lsp"`, { stdio: 'pipe' });
        return true;
    } catch (error) {
        return false;
    }
...
function startLanguageServer(pythonExecutable: string, pythonPath: string | undefined): void {
    ...
    const clientOptions: LanguageClientOptions = {
        ...
        synchronize: {
            // 监听工作区配置变化
            configurationSection: 'cuTilelsp',
 ...
 * 检查当前 Python 环境是否安装了 cutile_lsp，如果安装则启动服务器
...
async function tryStartLanguageServer(pythonPath: string | undefined, showNotification: boolean = true): Promise<boolean> {
    ...
    if (!isCutileLspInstalled(pythonExecutable)) {
        currentPythonPath = undefined;

        if (showNotification) {
            window.showWarningMessage(
                'Python library `cutile_lsp` is not installed in current environment. ' +
                'Install it or select a Python environment with it installed.'
            );
        }
        console.log(`cutile_lsp not installed in: ${pythonExecutable}`);
        return false;
    }
    ...
    if (showNotification && currentPythonPath !== pythonPath) {
        window.showInformationMessage('cuTile lsp Language Server started successfully.');
    }
...
export async function activate(context: ExtensionContext) {
    ...
    // 先注册 Python 解释器变化监听器（无论是否安装 cutile_lsp）
    ...
    console.log('cutile_lsp extension activated');
}
