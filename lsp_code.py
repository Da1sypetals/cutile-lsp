...
if __name__ == "__main__":
    ...
    def launc_layer_norm_fwd(_hints, _diagnostics):
        try:
            args = (Tensor((1024, 2048), dtype="float16"), Tensor((2048,), dtype="float16"), Tensor((2048,), dtype="float16"), Tensor((1024, 2048), dtype="float16"), Tensor((1024,), dtype="float32"), Tensor((1024,), dtype="float32"), 1e-5, 1024)
            type_info = check_semantics_and_type(layer_norm_fwd, args)
            ...
            loc_info = {
                "message": f"Unexpected error in kernel 'layer_norm_fwd': {error_msg}",
                "line": 118,
                "col": 0,
                "filename": "/root/cutile-lsp/kernel.py"
            }
            _diagnostics.append(loc_info)


    def launc_gemv_silu_mul_split_k_kernel(_hints, _diagnostics):
        try:
            args = (Tensor((1, 1536), dtype="float16"), Tensor((8960, 1536), dtype="float16"), Tensor((8960, 1536), dtype="float16"), Tensor((1, 8960), dtype="float16"), Tensor((2, 8960), dtype="float32"), Tensor((280,), dtype="int32"), 32, 64, 8, True)
            type_info = check_semantics_and_type(gemv_silu_mul_split_k_kernel, args)
            _hints.extend(type_info)
        except TileError as e:
            # 序列化Loc信息，只包含必要的字段
            loc_info = {
            ...
            error_msg = str(e).replace('"', "'").replace('\n', ' ')
            loc_info = {
                "message": f"Unexpected error in kernel 'gemv_silu_mul_split_k_kernel': {error_msg}",
                "line": 200,
                "col": 0,
                "filename": "/root/cutile-lsp/kernel.py"
            }
    ...
    launc_layer_norm_fwd(_hints, _diagnostics)
    launc_gemv_silu_mul_split_k_kernel(_hints, _diagnostics)

    # Save hints, code is indented outside
    lsp_code_exec_results = dict(hints=_hints, diagnostics=_diagnostics)
    with open("/root/.cutile_lsp/test/lsp_code_exec_results.json", "w") as f:
